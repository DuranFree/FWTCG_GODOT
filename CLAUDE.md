...
| CardDatabase | 100% |
| AIManager | ~98% |
| CombatManager | ~98%（法术对决已实现）|
| SpellManager | ~98%（33效果+9入场全实现）|
| KeywordManager | ~95% |
| LegendManager | ~100% |
| PromptManager | ~80% |
| GameBoard UI | ~97% |
...
## 已解决问题（本轮）
1. ✅ `GAME_MODE` 已为 `true`（早已修复，CLAUDE.md 过时）
2. ✅ 装备装配目标选择 — SpellManager 中已接入 PromptManager
3. ✅ 待命(Standby) UI — 已实现部署+显示+点击打出+AI待命
4. ✅ 游走(Roam) UI — 已实现移动目标选择
5. ✅ 法术对决系统 — `_run_duel_phase` + 对决Pass按钮 + AI对决响应
6. ✅ 友方法术目标选择 — `_target_uids` 系统支持ally/enemy/all三类目标
7. ✅ 符文多选交互 — 左键多选横置/右键多选回收，横置全部按钮复用为确定
8. ✅ 倒计时超时 bug — `end_action()` 不存在 → 修正为 `player_end_turn()`
9. ✅ 符文显示溢出 — HFlowContainer 双排布局，去掉回收子按钮节省空间
10. ✅ Git 仓库关联 — 推送至 https://github.com/DuranFree/FWTCG_GODOT.git
7. ✅ AI法术目标缺失 — 补充了 `buff_draw`、`deal1_same_zone`
8. ✅ `reveal_standby` bug — 修复了追加wrapper dict而非card的错误
9. ✅ 逻辑终审 — 全卡图与DB文字比对完成；虚空之门deal_damage已实现+1；力量方尖碑首回合限定+敌方支持已修复；攀圣长阶征服误触发已修复

## UI/体验提升进度（2026-03-18）
- ✅ [M2] 贴图资源整合 — ksha×34 + jiansheng×24 + runes×4 + coins + bgm 全部就位
- ✅ [M3] Log 面板重构 — 右侧全高(0-720) + 折叠按钮 + RichTextLabel 彩色分类
- ✅ [M1] 硬币先后手 + Mulligan 重置牌 — 硬币翻转动画+换牌界面，_ready()已接入
- ⬜ [M4] 拖拽出牌（基础框架已有，ghost视觉待补）
- ✅ [M5] 入场/战斗动画 — 单位弹入+得分弹跳+战斗闪色+屏幕震动+伤害浮字
- ✅ [M6] 粒子特效 — 金蓝双层背景粒子+点击涟漪+手牌入场动画
- ✅ [M7] 音效系统 — BGM+程序化SFX(出牌/战斗/得分/伤害/抽牌/胜负)

## 剩余待完善
- `force_move` 战场选择 UI（已用 PromptManager.ask() 实现，功能可用）
- 法术对决中传奇迅捷技能（已在 _on_legend_click 接入，功能可用）
